...
const initAudioSlideshow = function (Reveal) {
  ...
  function setupAllAudioElements(container, h, v, slide) {
    ...
    var fragments;
    while (
      (fragments = slide.querySelectorAll('.fragment[data-fragment-index="' + i + '"]')).length > 0
    ) {
      var audio = null;
      var video = null;
      var text = "";
  ...
  function linkVideoToAudioControls(audioElement, videoElement) {
    ...
    var videoSilence = new SilentAudio(Math.round(videoElement.duration + 0.5)); // create the wave file
      ...
      if (typeof Recorder == "undefined" || !Recorder.isRecording) {
        ...
        if (typeof indices.f !== "undefined" && indices.f >= 0) {
          var fragment = slide.querySelector(
            '.fragment[data-fragment-index="' + indices.f + '"][data-audio-advance]'
          );
          if (fragment) {
  ...
  Encode: function (e) {
    ...
    if (h > 0) {
      var s = (252 & e[t]) >> 2,
        i = (3 & e[t]) << 4;
      if (
        (h > 1 && (i |= (240 & e[++t]) >> 4), (a += this.chars[s]), (a += this.chars[i]), 2 == h)
      ) {
        var r = (15 & e[t++]) << 2;
